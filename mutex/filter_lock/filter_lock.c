...
void filter_lock(int thread_id, int n_threads) {
  for (int L = 1; L < n_threads; L++) { // n-1 levels
    atomic_store_explicit(&level[thread_id], L, memory_order_relaxed);
    atomic_store_explicit(&victim[L], thread_id, memory_order_release);
    ...
    for (;;) {
      ...
      if (!conflict ||
          atomic_load_explicit(&victim[L], memory_order_relaxed) != thread_id) {
        break;
      }
    }
  }
